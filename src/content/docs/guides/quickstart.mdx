---
title: Quickstart
description: Install fabric, sign in, connect a machine, and publish a local service — in under five minutes.
sidebar:
  order: 1
---
...
This installs three binaries: `fabric` (the CLI you'll use), `afd` (the background
...
`sudo` is not optional here: bringing up the tunnel creates a WireGuard
interface and installs the service that restores it after a reboot, which needs
administrator rights on every OS. `-E` keeps your environment so the CLI uses
your own `~/.fabric`, and it hands ownership of what it writes back to you —
every later command runs without `sudo`.
...
- **[CLI reference](/reference/cli/)** — every `fabric` command and flag.
